def scramble_word(word):
    ...
    if len(element) == 2:                                   # if middle letters are the same e.g "oo" in "good"
...
with open("talk.txt","r") as talk:
    with open("output_talk", "w") as scrambled_output:
        for line in talk:
            line = line.rstrip("\n")
            row = line.split("\t")
            ...
            for data in row:
                ...
                final_row = " ".join(scrambled_row)
                scrambled_output.write(f"{final_row}\n")
